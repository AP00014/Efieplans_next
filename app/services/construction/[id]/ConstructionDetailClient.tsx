...
const ConstructionDetailClient = () => {
  const params = useParams();
  const router = useRouter();
  const id = params.id as string;
  const [media, setMedia] = useState<ConstructionMedia | null>(null);
  const [relatedMedia, setRelatedMedia] = useState<ConstructionMedia[]>([]);
  ...
  if (error || !media) {
    return (
      <div className="construction-detail-error">
        ...
        <p>The requested construction media could not be found.</p>
        <Link href="/services/construction" className="back-link">
          <FaArrowLeft /> Back to Construction Gallery
        </Link>
      </div>
    );
  }
      ...
      <div className="construction-detail-header">
        <div className="container-custom">
          <div className="detail-header-actions">
            <Link href="/services/construction" className="detail-back-button">
              <FaArrowLeft /> Back
            </Link>
          </div>
        </div>
            ...
            <div className="construction-related-grid">
              {relatedMedia.map((item) => (
                <motion.div
                  ...
                  onClick={() => router.push(`/services/construction/${item.id}`)}
                >
                  <OptimizedImage
...
export default ConstructionDetailClient;
