...
const DesignGalleryClient = () => {
  const params = useParams();
  const category = params.category as string;
  const [images, setImages] = useState<DesignMedia[]>([]);
  ...
  useEffect(() => {
    if (category) {
      const categoryMedia = getMediaByCategory(category as DesignCategory);
  ...
  const categoryInfo = category ? getCategoryInfo(category as DesignCategory) : null;
  ...
  if (!categoryInfo || images.length === 0) {
    return (
      <div className="construction-gallery-error">
        ...
        <Link href="/services/design" className="back-link">
  ...
  return (
    <div className="construction-gallery-page">
      ...
      <div className="construction-gallery-header">
        <Link href="/services/design" className="gallery-back-button">
          <FaArrowLeft /> Back
        </Link>
...
export default DesignGalleryClient;
